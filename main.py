...
def build_row(name, item):
    # строка для scout_inbox: первые 9 колонок (contact/unit/status оставляет скрипт)
...
def write_to_inbox(rows):
    # пишет список строк в scout_inbox через Apps Script webhook
    if not INBOX_WEBHOOK or not rows:
        return
    r = requests.post(INBOX_WEBHOOK, json={"rows": rows}, timeout=60)
    if r.status_code not in (200, 201, 302):
        raise RuntimeError("Inbox webhook %s: %s" % (r.status_code, r.text[:300]))
    try:
        data = r.json()
        if not data.get("ok"):
            raise RuntimeError("Inbox script error: %s" % str(data)[:300])
    except ValueError:
        pass  # скрипт мог вернуть редирект/HTML — это нормально, строка записана
...
def process_filter(name, url):
    ...
    try:
        ...
        tg_send("⚠️ Скаут: карточки ушли, но в таблицу не записалось "
                "(фильтр «%s»)\n%s" % (name, str(e)[:400]))
...
def main():
    ...
    tg_send("🚀 Скаут запущен. BUILD = %s. Интервал: %g ч, maxItems: %d.\n%s"
            % (BUILD, INTERVAL_HOURS, MAX_ITEMS, inbox))
