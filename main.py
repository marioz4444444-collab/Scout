...
# как часто поглядывать, не добавлен ли новый фильтр (сек). Чтение таблицы бесплатно.
NEW_FILTER_CHECK_SEC = int(os.environ.get("NEW_FILTER_CHECK_SEC", "120"))
...
def process_filter(name, url):
    ...
    # --- ПЕРВЫЙ ПРОГОН НОВОГО ФИЛЬТРА ---
    # Заливаем всё, что нашли, в scout_inbox: для тебя это новые объекты,
    # даже если на Property Finder они висят давно. Карточки в Telegram при этом
    # НЕ шлём — иначе прилетит 20 штук подряд. Дальше — только новое.
    if stored is None:
        rows = [build_row(name, current[k]) for k in current]
        ok = write_to_inbox(rows) if rows else True

        if ok:
            kv_put(seen_key(name), list(current.keys()))
            tg_send("✅ Новый фильтр «%s»: залил %d листингов в таблицу.\n"
                    "Дальше шлю только новое." % (name, len(rows)))
        else:
            # в таблицу не записалось — НЕ помечаем виденными,
            # повторим на следующем прогоне, ничего не потеряется
            tg_send("⚠️ Новый фильтр «%s»: нашёл %d листингов, но в таблицу "
                    "НЕ записалось. Повторю на следующем прогоне."
                    % (name, len(rows)))
        return
...
def attempt_key(name):
    safe = re.sub(r"[^a-zA-Z0-9_]", "_", name.lower())
    return ("firsttry_" + safe)[:200]


def check_new_filters():
    """
    Проверка «не появился ли новый фильтр» — вызывается часто, между прогонами.
    Читать вкладку scout_filters бесплатно (обычная гугл-таблица), поэтому
    смотреть можно хоть каждую минуту.

    Новый фильтр = тот, по которому ещё нет памяти (seen). Такой прогоняем
    СРАЗУ, не дожидаясь 10:30/14:00/18:00 — первый прогон заливает всё
    найденное в scout_inbox.

    Платный запрос к Apify уходит только по новому фильтру и только один раз.
    Если запись в таблицу сорвётся — повторим не раньше чем через час,
    чтобы не жечь деньги в цикле.
    """
    try:
    ...
    for name, url in filters:
        try:
            if kv_get(seen_key(name)) is not None:
                continue                      # фильтр уже известен

            # не долбимся чаще раза в час, если прошлая попытка не удалась
            last = kv_get(attempt_key(name))
            if last:
                try:
                    if time.time() - float(last.get("ts", 0)) < 3600:
                        continue
                except Exception:
                    pass
            kv_put(attempt_key(name), {"ts": time.time()})

            tg_send("🆕 Вижу новый фильтр «%s» — прогоняю сразу, "
                    "не жду расписания." % name)
            process_filter(name, url)
        except Exception as e:
            tg_send("⚠️ Скаут: ошибка на новом фильтре «%s»\n%s"
                    % (name, str(e)[:400]))
...
def main():
    ...
    tg_send("🚀 Скаут запущен. BUILD = %s\n"
            "🕒 Расписание (Дубай): %s\n"
            "🆕 Новый фильтр подхвачу сразу, не дожидаясь расписания\n"
            "maxItems: %d\n%s\n"
            "Ближайший прогон: %s"
    ...
    while True:
        ...
        # между прогонами поглядываем: не добавил ли ты новый фильтр
        if time.time() - last_check >= NEW_FILTER_CHECK_SEC:
